var MSP = function (json) {
    ...
    uvmap(capemesh, 0, 1 * cape_scale, 1 * cape_scale, 10 * cape_scale, 16 * cape_scale); // Front side
    uvmap(capemesh, 1, 12 * cape_scale, 1 * cape_scale, 10 * cape_scale, 16 * cape_scale); // Back side
    uvmap(capemesh, 2, 1 * cape_scale, 0 * cape_scale, 10 * cape_scale, 1 * cape_scale); // Top edge
    uvmap(capemesh, 3, 11 * cape_scale, 0 * cape_scale, 10 * cape_scale, 1 * cape_scale, 1); // Bottom edge
    uvmap(capemesh, 4, 0 * cape_scale, 1 * cape_scale, 1 * cape_scale, 16 * cape_scale); // Left edge
    uvmap(capemesh, 5, 11 * cape_scale, 1 * cape_scale, 1 * cape_scale, 16 * cape_scale); // Right edge
    ...
    skin.onload = function () {
        ...
        var isPixelDifferent = function (x, y) {
            if (pixels[(x + y * WIDTH) * 4 + 0] !== pixels[colorIndex + 0] || pixels[(x + y * WIDTH) * 4 + 1] !== pixels[colorIndex + 1] || pixels[(x + y * WIDTH) * 4 + 2] !== pixels[colorIndex + 2] || pixels[(x + y * WIDTH) * 4 + 3] !== pixels[colorIndex + 3]) {
                return true;
            }
            return false;
        };
        ...
        for (var i = 0; i < WIDTH; i += 1) {
            for (var j = 0; j < HEIGHT; j += 1) {
                ...
                if (isOnecolor && ((i >= HEIGHT && i < WIDTH && j >= 8 && j < 16) || (i >= 40 && i < 56 && j >= 0 && j < 8))) {
                    pixels[(i + j * WIDTH) * 4 + 3] = 0;
                }
                sbc.fillStyle = 'rgba(' + pixels[(i + j * WIDTH) * 4 + 0] + ', ' + pixels[(i + j * WIDTH) * 4 + 1] + ', ' + pixels[(i + j * WIDTH) * 4 + 2] + ', ' + pixels[(i + j * WIDTH) * 4 + 3] / 255 + ')';
    ...
    return {
        ...
        getPrice: function (startPrice) {
            ...
            if (mod == 1)
                return startPrice * a[WIDTH];
            else
                return startPrice * a[CAPE_WIDTH];
        },
        getSize: function () {
            var mod = document.getElementById("mod").value;
            if (mod == 1)
                ...
                return CAPE_WIDTH + "x" + CAPE_HEIGHT;
        }
    };
};
